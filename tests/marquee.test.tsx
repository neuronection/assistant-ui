...
import {
  ...
  rectsIntersect,
  useMarquee,
} from '../src/components/marquee/Marquee'

describe('rect math', () => {
  it('rectsIntersect detects overlap and non-overlap', () => {
  ...
  it('marqueeRect normalizes drag direction', () => {
...
describe('hitTestIds', () => {
  it('returns ids of elements intersecting the band', () => {
...
describe('useMarquee', () => {
  ...
  it('drag over empty background selects intersecting items', () => {
  ...
  it('a click without drag clears the selection', () => {
    const onSelect = vi.fn()
    const { getByTestId } = render(<MarqueeHost onSelect={onSelect} />)
    const pane = getByTestId('pane')
  ...
  it('mousedown on an item does not start a marquee', () => {
    const onSelect = vi.fn()
  ...
  it('sub-threshold movement never arms the marquee', () => {
    const onSelect = vi.fn()
    const { getByTestId } = render(<MarqueeHost onSelect={onSelect} />)
    const pane = getByTestId('pane')
  ...
  it('ctrl drag unions with the base selection', () => {
    const onSelect = vi.fn()
    ...
    pane.dispatchEvent(mouseEvent('mousedown', 0, 0, { ctrlKey: true }))
    window.dispatchEvent(mouseEvent('mousemove', 50, 10))
    window.dispatchEvent(mouseEvent('mouseup', 50, 10))

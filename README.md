...
| [`modal`](./docs/components/modal.md) (`Modal` + parts incl. `ModalBody`, `PanelModal`) | `Modal` + parts (compound Radix dialog) · `PanelModal` (header/body/footer shell, full-screen on mobile) | [stories](https://neuronection.github.io/assistant-ui/?story=modal--sizes) |
...
| [`rich-text-editor`](./docs/components/rich-text-editor.md) | `RichTextEditor` — controlled tiptap markdown editor: full-override `extensions`, configurable toolbar groups + per-level headings + `toolbarExtra` slot, `onReady`/`contentClassName` composition props, label/icon props, caret-preserving external sync | [stories](https://neuronection.github.io/assistant-ui/?story=rich-text-editor--default) |
...
| [`markdown-diff-view`](./docs/components/markdown-diff-view.md) | `MarkdownDiffView` — side-by-side **formatted** markdown diff (KaTeX/tables/diagrams render; block-level compare, change navigation, folds, + `splitMarkdownBlocks`) | [stories](https://neuronection.github.io/assistant-ui/?story=markdown-diff-view--default) |
...
| [`segmented-tabs`](./docs/components/segmented-tabs.md) | `SegmentedTabs` (pill tablist with sliding thumb, arrow-key activation, disabled-skip) | [stories](https://neuronection.github.io/assistant-ui/?story=segmented-tabs--three-tabs) |
| [`error-banner`](./docs/components/error-banner.md) / [`undo-notice`](./docs/components/undo-notice.md) / [`empty-state`](./docs/components/empty-state.md) / [`spinner`](./docs/components/spinner.md) / [`skeleton`](./docs/components/skeleton.md) | presentational feedback: alert banner with `action` slot · undo toast · `EmptyState` · `Spinner` · `Skeleton`/`SkeletonText` shimmer placeholders (reduced-motion static) · rich hover previews: [`hover-card`](./docs/components/hover-card.md) | [stories](https://neuronection.github.io/assistant-ui/?story=hover-card--basic)(https://neuronection.github.io/assistant-ui/?story=feedback--errors) |
...
| [`chat-message`](./docs/components/chat-message.md) / [`chat-reasoning`](./docs/components/chat-reasoning.md) / [`chat-tool-card`](./docs/components/chat-tool-card.md) / [`chat-turn-status`](./docs/components/chat-turn-status.md) | `ChatMessage` (bubble: actions row, variant switcher, edit-and-resend, error/interrupted states) · `ChatReasoning` (collapsible thinking block) · `ChatToolCard` (tool observation: status, duration, args/result, app-provided result views via `renderResult`) · `ChatTurnStatus` (live pre-text turn status: dots, phase label, elapsed timer) | [stories](https://neuronection.github.io/assistant-ui/?story=chat-message--assistant) |
| [`chat-hitl`](./docs/components/chat-hitl.md) | `HitlProposalCard` / `FieldDiff` — human-in-the-loop proposal cards: field-level before/after diffs (long text via TextDiffView, structured collection chips), approve/reject with armed destructive confirm, preview slot, pending/approved/rejected/conflict/expired/reverted states | [stories](https://neuronection.github.io/assistant-ui/) |
| [`chat-composer`](./docs/components/chat-composer.md) | `ChatComposer` — auto-growing IME-safe input with send/stop states, toolbar/attachment/suggestion slots, drag-drop + paste wiring, `data-multiline` row styling hook | [stories](https://neuronection.github.io/assistant-ui/?story=chat-composer--default) |
...
| [`chat-tools-catalog`](./docs/components/chat-tools-catalog.md) | `ChatToolsCatalog` — searchable disclosure catalog of the tools an assistant can use (name, title, scope, arguments, example, response); fetching stays app-side | [stories](https://neuronection.github.io/assistant-ui/?story=chat-tools-catalog--default) |
| [`chat-export`](./docs/components/chat-export.md) | `buildChatMarkdown` / `chatExportFileName` / `downloadChatMarkdown` — conversation → Markdown export with role-label styles and app-supplied blockquote annotations | [stories](https://neuronection.github.io/assistant-ui/?story=chat-export--bold-style) |
| [`chat-history-button`](./docs/components/chat-history-button.md) | `ChatHistoryButton` — labelled history popover for a chat header (open-refresh hook, close-on-pick); the session list stays app-side | [stories](https://neuronection.github.io/assistant-ui/?story=chat-history-button--default) |
| [`chat-trace-meta`](./docs/components/chat-trace-meta.md) | `ChatTraceMeta` — compact `model · duration · N tools` trace badges; renders nothing without data | [stories](https://neuronection.github.io/assistant-ui/?story=chat-trace-meta--default) |
| [`chat-trace-timeline`](./docs/components/chat-trace-timeline.md) |
| [`detail-value`](./docs/components/detail-value.md) | `DetailValue` — internal parsed-detail pane (key/value rows, value chips, preformatted fallback) used by chat-tool-card + chat-trace-timeline | [stories](https://neuronection.github.io/assistant-ui/) |
 `ChatTraceTimeline` — collapsible per-turn trace: duration-proportional phase/tool bars, tokens, model, reasoning disclosure | [stories](https://neuronection.github.io/assistant-ui/?story=chat-trace-timeline--default) |
...
| [`flow-trace`](./docs/components/flow-trace.md) | `FlowTraceCard` (run ledger over the library-owned `FlowTrace` schema: outcome badge, stage timeline, per-call LLM table with tokens/latency/prompt version, ops ✓/✗ list, simulated badge for mock rows) + `FlowTelemetryStrip` (compact `calls · tokens in/out · edits` live counters) | [stories](https://neuronection.github.io/assistant-ui/?story=flow-trace--completed-run) |

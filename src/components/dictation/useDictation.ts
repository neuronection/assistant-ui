...
export interface UseDictationOptions {
  /** App-owned transport: upload the blob, resolve with the transcript. */
  transcribe: (audio: Blob) => Promise<DictationResult>
  /** Map a transport failure to a structured error (e.g. 409 → 'unassigned'). */
  classifyError?: (error: unknown) => DictationError
...
export function useDictation(options: UseDictationOptions) {
  ...
  const stop = useCallback(async () => {
    ...
    try {
      ...
      const result = await optionsRef.current.transcribe(blob)
      setStatus('idle')
      optionsRef.current.onResult?.(result.text)
    } catch (transcribeError) {
      const classified =
        optionsRef.current.classifyError?.(transcribeError) ??
        ({
          kind: 'failed',
          detail:

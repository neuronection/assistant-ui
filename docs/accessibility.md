...
| `TaskAssignmentPicker` | task rows with per-row `ModelPicker` + clear button | clear button empties the assignment | clean |
| `ConnectionTestRow` | status text (`role="status"` while testing); error message | Enter on *Test* fires `onTest` | clean |

...
| `SidebarNav` | `<nav aria-label>`; active entry `aria-current="page"`; group triggers `aria-expanded` + `aria-controls`; rail flyout content labelled by its group | Tab traverses in order; Enter/Space activates (leaves navigate, groups toggle); ArrowUp/Down move between visible items; ArrowRight expands/enters a group; ArrowLeft collapses/returns to the trigger; Home/End jump; rail flyout: opens on hover/click/Enter, ArrowUp/Down within, Escape closes and focus returns to the trigger | clean (expanded, collapsed, flyout open) |
